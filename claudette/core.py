# AUTOGENERATED! DO NOT EDIT! File to edit: ../00_core_new.ipynb.
...
def mk_toolres(
    ...
    return_func_output=False
    ):
    "Create a `tool_result` message from response `r`."
    cts = getattr(r, 'content', [])
    res = [mk_msg(r)]
    func_outputs = [(o.id, call_func(o, ns=ns, obj=obj)) for o in cts if isinstance(o,ToolUseBlock)]
    tcs = [mk_funcres(idx, o) for idx, o in func_outputs]
    if tcs: res.append(mk_msg(tcs))
    if return_func_output: return res, func_outputs
    return res
...
class Chat:
    def __init__(self,
                 ...
                 tool_choice:Optional[dict]=None, # force use of some tool
                 return_func_output: bool=False): # return the outputs of tools
        "Anthropic chat client."
        assert model or cli
        assert cont_pr != "", "cont_pr may not be an empty string"
        self.c = (cli or Client(model))
        self.h,self.sp,self.tools,self.cont_pr,self.tool_choice,self.return_func_output = [],sp,tools,cont_pr,tool_choice,return_func_output
...
def __call__(self:Chat,
    ...
    if self.return_func_output:
        toolres, func_outputs = mk_toolres(res, ns=self.tools, obj=self, return_func_output=self.return_func_output)
    else:
        toolres = mk_toolres(res, ns=self.tools, obj=self)
    self.h += toolres
    if self.return_func_output: return toolres, func_outputs
    return res

...
# %% auto 0
__all__ = ['empty', 'models', 'models_aws', 'models_goog', 'find_block', 'contents', 'usage', 'mk_msgs', 'find_type', 'Client',
...
# %% ../00_core.ipynb
def find_type(msgs, msg_type:type):
    "Helper to find the first occurance of a specific message type in a list of messages"
    return first((first(o.content) for o in msgs if not isinstance(o.content, str)), lambda x: x.type == msg_type)
...
def mk_funcres(tuid, res):
    ...
    return AttrDict(type="tool_result", tool_use_id=tuid, content=res)
...
class Chat:
    ...
    @property
    def last_tool_result(self): return find_type(reversed(self.h), msg_type="tool_result")
...
def img_msg(data:bytes, cache=False)->dict:
    ...
    return AttrDict(_add_cache({"type": "image", "source": r}, cache))

# %% ../00_core.ipynb
def text_msg(s:str, cache=False)->dict:
    "Convert `s` to a text message"
    return AttrDict(_add_cache({"type": "text", "text": s}, cache))
...
def mk_msg(content, # A string, list, or dict containing the contents of the message
    ...
    return AttrDict(role=role, content=content, **kw)

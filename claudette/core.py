...
# %% auto 0
__all__ = ['empty', 'models', 'models_aws', 'models_goog', 'find_block', 'contents', 'usage', 'mk_msgs', 'short', 'count_images',
           'get_text', 'summary', 'CLogger', 'Client', 'mk_tool_choice', 'call_func', 'mk_toolres', 'Chat', 'img_msg',
           'text_msg', 'mk_msg']
...
# %% ../00_core.ipynb 29
@patch
def _repr_html_(self:(Message)):
    det = '\n'.join(f'<li>{k}: {v}</li>' for k,v in self.model_dump().items())
...
def short(s):
    s = s.strip()
    return s[:100] + '...' if len(s)>50 else s

def count_images(msgs):
    i = 0
    if msgs is None: return 0
    for m in msgs:
        if isinstance(m, dict) and isinstance(m.get('content'), list):
            for c in m["content"]:
                if isinstance(c, dict) and c.get("type") == "image": i += 1
    return i

def get_text(msgs):
    if msgs is None: return ""
    for m in msgs:
        if isinstance(m, str):                  return short(m)
        if isinstance(m.get('content'), str):   return short(m['content'])
        if isinstance(m.get('content'), list):
            for c in m['content']:
                if isinstance(c, dict) and len(c.get("text", "")) > 0:
                    return short(c["text"])
    return ""

def summary(msgs, tools):
    summary = ""
    ni = count_images(msgs)
    if ni == 1: summary += f" (image)"
    elif ni > 1: summary += f" (image x{ni})"

    nt = 0 if not isinstance(tools, list) else len(tools)
    if nt == 1: summary += " (tool)"
    elif nt > 1: summary += f" (tool x{nt})"

    return summary + f" {get_text(msgs)}"

class CLogger():
    def __init__(self): self.log = []
    def append(self, data): self.log.append(data)
    def __getitem__(self, i): return self.log[i]
    def __len__(self): return len(self.log)
    def __repr__(self): return f"[#{len(self)}] {repr(self.log)}"
    def _repr_html_(self):
        logs = "Empty" if not len(self.log) else ""
        for log in self.log:
            nmsg = len(log.get("msgs", [])) if isinstance(log.get("msgs"), list) else 0
            det = ''.join(f'<li>{html.escape(repr(k))}: {html.escape(repr(v))}</li>' for k,v in log.items())
            logs += f"""
<details>
<summary style="display: list-item">[# {nmsg}] { summary(log.get("msgs", []), log.get("tools", [])) }</summary>
<ul>{det}</ul>
</details>
        """
        return logs
